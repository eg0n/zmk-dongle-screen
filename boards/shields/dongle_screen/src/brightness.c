...
void screen_animation_thread(void*, void*, void*) {
  ...
  while (1) {
    ...
    LOG_DBG("screen animation update: %d -> %d / %d", current_brightness, set_brightness, ambient_deflection);
    uint8_t target_brightness = set_brightness == 0 ? 0 : CLAMP_BL(set_brightness + ambient_deflection);
    ...
    LOG_DBG("screen animation update: duration %d, delay %d, steps %d", duration_ms, delay_us, steps);
    uint8_t last_applied = 255;
    /* animate! */
    for (uint8_t i = 0; i <= steps; i++) {
        float t = (float)i / steps;
        float func_val = fade_function(t);
        float interpolated = current_brightness + (target_brightness - current_brightness) * func_val;
...
K_THREAD_DEFINE(animation_tid, 768, screen_animation_thread, NULL, NULL, NULL, 6, 0, 0);

static void update_screen(screen_state_t new_screen_state, uint8_t new_brightness) {
  k_mutex_lock(&fade_mutex, K_FOREVER);
  ...
  while (1) {
    if (screen_state == SCREEN_ON) {
      ...
      if (remaining <= 0) {
        ...
        /* sleep exactly as long as needed until timeout or next activity */
...
static int key_listener(const zmk_event_t *eh) {
  ...
  if (ev && ev->state) {
    ...
    if (ev->keycode == CONFIG_DONGLE_SCREEN_BRIGHTNESS_UP_KEYCODE) {
      ...
      new_brightness = CLAMP_BL(set_brightness + CONFIG_DONGLE_SCREEN_BRIGHTNESS_STEP);
...
static void ambient_light_thread(void) {
  ...
  while (1) {
    ...
    if (!sensor_sample_fetch(ambient_sensor) &&
      ...
      /* Limit the value between 0 and the configured max, then scale that reading
       * to a float between -0.5 and 0.5
       */
      uint8_t sensor_reading = CLAMP(val.val1, 0, CONFIG_DONGLE_SCREEN_AMBIENT_SENSOR_MAX_VALUE);      
      ambient_deflection = (int8_t)(
        ((float)sensor_reading / CONFIG_DONGLE_SCREEN_AMBIENT_SENSOR_MAX_VALUE - 0.5) * 
      ...
      last_val = val;
      LOG_DBG("Sensor reading: %d, deflection %d", sensor_reading, ambient_deflection);
    }
    k_sleep(K_MSEC(AMBIENT_SENSOR_EVALUATION_PERIOD_MS));
  }
}

K_THREAD_DEFINE(ambient_light_tid, 512, ambient_light_thread, NULL, NULL, NULL, 6, 0, 0);
